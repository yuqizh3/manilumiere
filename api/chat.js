...
export default async function handler(req, res) {
  ...
  try {
    const body = req.body || {};
    // 语言：en 时要求 AI 用英文输出（保留卡片结构与排版）
    const langNote = body.lang === 'en'
      ? '\n\nIMPORTANT: Write your entire response in natural, warm, fluent English. Keep the same card structure and layout.'
      : '';
    const response = await fetch(`${AI_BASE_URL}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: (body.system || '') + langNote },
          ...(body.messages || [])
        ]
      })
    });
